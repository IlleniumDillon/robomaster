...
:nh_(nh), nh_private_(nh_private)  {
    ...
    odom_sub_ =
        nh_.subscribe<geometry_msgs::PoseStamped>("/airsim_node/drone_1/debug/pose_gt",1, &TrajectoryPublisherNode::odomCallback,this);
    ...
    velocity_pub = nh_.advertise<geometry_msgs::Twist>("/velocity_topic", 10);
...
void TrajectoryPublisherNode::desiredStatesPub() {
    ...
    if (t > trajDuration_) {
        cmd_.position.x = odom_->pose.position.x;
        cmd_.position.y = odom_->pose.position.y;
        cmd_.position.z = odom_->pose.position.z;
...
void TrajectoryPublisherNode::odomCallback(const geometry_msgs::PoseStampedConstPtr &msg) {
    odom_ = msg;
    ...
    twist_msg.linear.x = odom_vel_.x();
    twist_msg.linear.y = odom_vel_.y();
    twist_msg.linear.z = odom_vel_.z();

    // 发布消息
    if(twist_msg.linear.x !=0 && twist_msg.linear.y !=0 && twist_msg.linear.z !=0)
    ...
    currentPose_.pose.position = msg->pose.position;
    currentPose_.pose.orientation = msg->pose.orientation;
